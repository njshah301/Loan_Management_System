...
public class ItemServiceImpl implements ItemService{
	...
	public List<ItemDto> getItemsByCategory(String category) {
		List<ItemDto> itemDtos=  itemRepository.findByItemCategory(category).stream().map(item-> convertEntityToDto(item)).collect(Collectors.toList());
		return itemDtos;
	}
	
	public ItemDto getItemsByDescription (String description) {
		ItemDto itemDtos=  convertEntityToDto(itemRepository.findByItemDescription(description));
		return itemDtos;
	}
